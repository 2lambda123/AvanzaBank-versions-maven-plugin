 ...
 * @since 1.0-alpha-1
 */
@Mojo(name = "display-dependency-updates", requiresProject = true,
...
public class DisplayDependencyUpdatesMojo extends AbstractVersionsDisplayMojo {
  ...
  /**
   * Whether to process the dependencies sections of plugins which are defined
   * in pluginManagement.
   *
   * @since 2.5
   */
  ...
  /**
   * Whether to show additional information such as dependencies that do not
   * need updating. Defaults to false.
   *
   * @since 2.1
   */
  ...
  private static Set<Dependency>
  extractPluginDependenciesFromPluginsInPluginManagement(Build build) {
  ...
  private static Set<Dependency>
  extractDependenciesFromPlugins(List<Plugin> plugins) {
    Set<Dependency> result = new TreeSet<>(new DependencyComparator());
  ...
  /**
   * Returns a set of dependencies where the dependencies which are defined in
   * the dependency management section have been filtered out.
   *
   * @param dependencies The set of dependencies.
   * @param dependencyManagement The set of dependencies from the dependency
   *     management section.
   * @return A new set of dependencies which are from the set of dependencies
   *     but not from the set of dependency
   *         management dependencies.
   * @since 1.0-beta-1
   */
  private static Set<Dependency>
  removeDependencyManagment(Set<Dependency> dependencies,
  ...
  // open for tests
  protected static boolean dependenciesMatch(Dependency dependency,
    ...
    if (managedDependency.getClassifier() == null ||
        Objects.equals(managedDependency.getClassifier(),
  ...
  public boolean isProcessingDependencies() { return processDependencies; }
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    Set<Dependency> dependencyManagement =
        new TreeSet<>(new DependencyComparator());
    if (getProject().getDependencyManagement() != null) {

      List<Dependency> dependenciesFromPom =
          getProject().getDependencyManagement().getDependencies();
      for (Dependency dependency : dependenciesFromPom) {
        getLog().debug("dependency from pom: " + dependency.getGroupId() + ":" +
        ...
        if (dependency.getVersion() == null) {
          ...
          if (getProject().hasParent()) {
            ...
            if (getProject().getParent().getDependencyManagement() != null) {
              List<Dependency> parentDeps = getProject()
                                                .getParent()
            ...
            String message = "We can't get the version for the dependency " +
                             dependency.getGroupId() + ":" +
                             dependency.getArtifactId() +
                             " cause there does not exist a parent.";
            getLog().error(message);
            // Throw error cause we will not able to get a version for a
            // dependency.
    ...
    if (isProcessingDependencyManagement()) {
      dependencies =
          removeDependencyManagment(dependencies, dependencyManagement);
    }
    ...
    if (isProcessingPluginDependencies()) {
      pluginDependencies =
          extractDependenciesFromPlugins(getProject().getBuildPlugins());
    }

    Set<Dependency> pluginDependenciesInPluginManagement =
        new TreeSet<>(new DependencyComparator());
    ...
    try {
      ...
      if (isProcessingDependencies()) {
        logUpdates(getHelper().lookupDependenciesUpdates(dependencies, false),
                   "Dependencies");
      }
      if (isProcessPluginDependenciesInDependencyManagement()) {
        logUpdates(getHelper().lookupDependenciesUpdates(
                       pluginDependenciesInPluginManagement, false),
                   "pluginManagement of plugins");
      }
      if (isProcessingPluginDependencies()) {
        logUpdates(
            getHelper().lookupDependenciesUpdates(pluginDependencies, false),
            "Plugin Dependencies");
      }
  ...
  private void logUpdates(Map<Dependency, ArtifactVersions> updates,
    ...
    while (i.hasNext()) {
      ...
      String left =
          "  " + ArtifactUtils.versionlessKey(versions.getArtifact()) + " ";
      final String current;
      ArtifactVersion latest;
      if (versions.isCurrentVersionDefined()) {
        current = versions.getCurrentVersion().toString();
        latest =
            versions.getNewestUpdate(calculateUpdateScope(), allowSnapshots);
      } else {
        ArtifactVersion newestVersion = versions.getNewestVersion(
            versions.getArtifact().getVersionRange(), allowSnapshots);
        current = versions.getArtifact().getVersionRange().toString();
        latest = newestVersion == null
                     ? null
                     : versions.getNewestUpdate(newestVersion,
                                                calculateUpdateScope(),
                                                allowSnapshots);
        if (latest != null &&
    ...
    if (isVerbose()) {
      if (usingCurrent.isEmpty()) {
        if (!withUpdates.isEmpty()) {
          logLine(false, "No dependencies in " + section +
                             " are using the newest version.");
          logLine(false, "");
        }
      } else {
        logLine(false, "The following dependencies in " + section +
                           " are using the newest version:");
    ...
    if (withUpdates.isEmpty()) {
      if (!usingCurrent.isEmpty()) {
        logLine(false,
                "No dependencies in " + section + " have newer versions.");
        logLine(false, "");
      }
    } else {
      logLine(false, "The following dependencies in " + section +
   ...
   * @throws org.apache.maven.plugin.MojoFailureException when things go wrong
   *     in a very bad way
   * @throws javax.xml.stream.XMLStreamException when things go wrong with XML
   *     streaming
   * @see
   *     org.codehaus.mojo.versions.AbstractVersionsUpdaterMojo#update(org.codehaus.mojo.versions.rewriting.ModifiedPomXMLEventReader)
   * @since 1.0-alpha-1
   */
