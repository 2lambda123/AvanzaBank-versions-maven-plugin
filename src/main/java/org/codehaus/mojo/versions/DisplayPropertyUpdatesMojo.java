 ...
 * @since 1.0-beta-1
 */
@Mojo(name = "display-property-updates", requiresProject = true,
      requiresDirectInvocation = true, threadSafe = true)
public class DisplayPropertyUpdatesMojo extends AbstractVersionsDisplayMojo {
   ...
   * @since 1.0-alpha-3
   */
  @Parameter private Property[] properties;
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    Map<Property, PropertyVersions> propertyVersions =
        this.getHelper().getVersionPropertiesMap(
            getProject(), properties, includeProperties, excludeProperties,
            autoLinkItems);
    for (Map.Entry<Property, PropertyVersions> entry :
         propertyVersions.entrySet()) {
      Property property = entry.getKey();
      PropertyVersions version = entry.getValue();

      final String currentVersion =
          getProject().getProperties().getProperty(property.getName());
      if (currentVersion == null) {
        continue;
      }

      int segment = determineUnchangedSegment(
          allowMajorUpdates, allowMinorUpdates, allowIncrementalUpdates);
      ArtifactVersion winner = version.getNewestVersion(
          currentVersion, property, this.allowSnapshots, this.reactorProjects,
          this.getHelper(), false, segment);
      ...
      if (winner != null && !currentVersion.equals(winner.toString())) {
        ...
        int padding = INFO_PAD_SIZE - currentVersion.length() -
                      newVersion.length() - 4 + getDisplayTerminalWidthOffset();
    ...
    if (updates.isEmpty() && current.isEmpty()) {
      ...
      logLine(
          false,
          "All version properties are referencing the newest version available.");
    }
