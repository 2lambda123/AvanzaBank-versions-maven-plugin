 ...
 * @since 1.0-beta-1
 */
@Mojo(name = "set", requiresProject = true, requiresDirectInvocation = true,
...
public class SetMojo extends AbstractVersionsUpdaterMojo {
   ...
   * @since 1.0-beta-1
   */
  @Parameter(property = "newVersion") private String newVersion;
   ...
   * @component
   */
  @Component private Prompter prompter;
  ...
  /**
   * Whether to add next version number and <code>-SNAPSHOT</code> to the
   * existing version.
   *
   * @since 2.10
   */
  ...
  private synchronized void addChange(String groupId, String artifactId,
                                      String oldVersion, String newVersion) {
    if (!newVersion.equals(oldVersion)) {
      sourceChanges.add(
          new VersionChange(groupId, artifactId, oldVersion, newVersion));
    }
  }
   ...
   *
   * @throws org.apache.maven.plugin.MojoExecutionException when things go
   *     wrong.
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    if (removeSnapshot && !nextSnapshot) {
      ...
      if (version.endsWith(SNAPSHOT)) {
        ...
        newVersion = release;
        getLog().info("SNAPSHOT found.  BEFORE " + version +
                      "  --> AFTER: " + newVersion);
      }
    }
    ...
    if (!removeSnapshot && nextSnapshot) {
      ...
      if (version.endsWith(SNAPSHOT)) {
        versionWithoutSnapshot =
            version.substring(0, version.indexOf(SNAPSHOT));
      }
      ...
      String nextVersion =
          StringUtils.join(numbers.toArray(new String[0]), ".");
    ...
    if (StringUtils.isEmpty(newVersion)) {
      if (settings.isInteractiveMode()) {
        try {
          newVersion =
              prompter.prompt("Enter the new version to set",
        ...
        throw new MojoExecutionException(
            "You must specify the new version, either by using the newVersion "
            +
    ...
    try {
      ...
      Map<String, Model> reactorModels =
          PomHelper.getReactorModels(project, getLog());
      ...
      getLog().info("Processing change of " + groupId + ":" + artifactId + ":" +
                    oldVersion + " -> " + newVersion);
      Pattern groupIdRegex = Pattern.compile(RegexUtils.convertWildcardsToRegex(
          fixNullOrEmpty(groupId, "*"), true));
      Pattern artifactIdRegex =
          Pattern.compile(RegexUtils.convertWildcardsToRegex(
              fixNullOrEmpty(artifactId, "*"), true));
      Pattern oldVersionIdRegex =
          Pattern.compile(RegexUtils.convertWildcardsToRegex(
              fixNullOrEmpty(oldVersion, "*"), true));
      boolean found = false;
      for (Model m : reactor.values()) {
        ...
        if (((groupIdRegex.matcher(mGroupId).matches() &&
          ...
          applyChange(project, reactor, files, mGroupId, m.getArtifactId(),
                      StringUtils.isBlank(oldVersion) || "*".equals(oldVersion)
                          ? ""
                          : m.getVersion());
        }
      }
  ...
  private void applyChange(MavenProject project,
                           SortedMap<String, Model> reactor, Set<File> files,
                           String groupId, String artifactId,
                           String oldVersion) {

    getLog().debug("Applying change " + groupId + ":" + artifactId + ":" +
                   oldVersion + " -> " + newVersion);
    // this is a triggering change
    addChange(groupId, artifactId, oldVersion, newVersion);
    // now fake out the triggering change

    final Map.Entry<String, Model> current =
        PomHelper.getModelEntry(reactor, groupId, artifactId);
    ...
    for (Map.Entry<String, Model> sourceEntry : reactor.entrySet()) {
      ...
      for (Map.Entry<String, Model> stringModelEntry :
           processAllModules ? reactor.entrySet() : //
               PomHelper
                   .getChildModels(reactor, sourceGroupId, sourceArtifactId)
                   .entrySet()) {
        ...
        if ((updateMatchingVersions || !targetExplicit) //
            && (parent != null &&
                StringUtils.equals(parent.getVersion(),
          ...
          getLog().debug("    will become " +
                         ArtifactUtils.versionlessKey(
                             ...
                             PomHelper.getArtifactId(targetModel)) +
                         ":" + sourceVersion);
          addChange(PomHelper.getGroupId(targetModel),
                    PomHelper.getArtifactId(targetModel),
  ...
  private void addFile(Set<File> files, MavenProject project,
                       String relativePath) {
  ...
  protected synchronized void update(ModifiedPomXMLEventReader pom)
    ...
    try {
      ...
      log.setContext("Processing " + PomHelper.getGroupId(model) + ":" +
  ...
  private void updateBuildOutputTimestamp(ModifiedPomXMLEventReader pom,
                                          Model model)
      throws XMLStreamException {
    String buildOutputTimestamp =
        model.getProperties().getProperty("project.build.outputTimestamp");
    ...
    PomHelper.setPropertyVersion(pom, null, "project.build.outputTimestamp",
